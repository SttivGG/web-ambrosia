...
import { categoryV1Schema } from '@ambrosia/contracts';
...
it('lectura usa cliente autenticado y ruta relativa Nginx', async () => {
  ...
  expect(
    await catalogRequest('categories/' + category.id, categoryV1Schema),
  ).toEqual(category);
  expect(authenticatedFetch).toHaveBeenCalledWith(
    '/api/inventory/catalog/categories/' + category.id,
...
it('mutación obtiene CSRF y envía expectedVersion sin repetir', async () => {
  ...
  await catalogRequest('categories/' + category.id, categoryV1Schema, 'PATCH', {
    expectedVersion: 1,
    name: 'Leche',
  });
...
it('conflicto conserva código y campos y no se reintenta', async () => {
  ...
  await expect(
    catalogRequest('categories/id', categoryV1Schema, 'PATCH', {}),
...
it('fallo de red no repite mutación de resultado ambiguo', async () => {
  ...
  await expect(
    catalogRequest('categories', categoryV1Schema, 'POST', {}),
...
it('rechaza respuestas inválidas sin exponer contenido', async () => {
  ...
  await expect(
    catalogRequest('categories', categoryV1Schema),
  ).rejects.toBeInstanceOf(CatalogRequestError);
});
it('403 sin error de catálogo usa mensaje seguro', async () => {
  vi.mocked(authenticatedFetch).mockResolvedValue(
    new Response(JSON.stringify({ message: 'private internals' }), {
      status: 403,
    }),
  );
  await expect(catalogRequest('categories', categoryV1Schema)).rejects.toThrow(
    'No tienes permiso',
  );
});
it('permite retorno al catálogo y rechaza escape del panel', () => {
  expect(safeReturnTo('/inventario/catalogo?active=true')).toBe(
    '/inventario/catalogo?active=true',
  );
  expect(safeReturnTo('/inventario/catalogo/../../api/auth/logout')).toBe(
    '/dashboard',
  );
  expect(safeReturnTo('/inventario/catalogootro')).toBe('/dashboard');
});

...
Fórmulas con revisiones inmutables, lotes, consumo, cierre lógico, cancelación compensatoria y trazabilidad. Coordinación persistente e idempotente entre Production e Inventory según ADR-010; se conserva ADR-002 sin atomicidad global. Interfaces /produccion/formulas y /produccion/lotes, permisos existentes y conflictos que conservan entradas. Ver production.md para API, migraciones, credencial técnica y recuperación. Matriz de validación completada; Fase 5 permanece pendiente.

Tres migraciones aditivas y dos respaldos verificados, sin modificar migraciones históricas. Doce operaciones HTTP públicas con production.read/write y tres rutas técnicas privadas en Inventory. OWNER/ADMIN/OPERATOR gestionan; VIEWER consulta. Production conserva borradores mientras Inventory no confirma el consumo; la recuperación reutiliza el UUID persistido y la compensación genera movimientos inversos sin borrar historial.

Cierre: 428 pruebas aprobadas (389 heredadas y 39 nuevas), siete grupos PostgreSQL/HTTP/Playwright y 18 grupos de test:stack. Recuperación comprobada antes y después del consumo, incluida caída al actualizar Production; carreras de stock sin saldo negativo, compensación idempotente y ledger consistente. Panel comprobado a 1440/768/375, con conflictos que conservan datos. Datos originales intactos, fixtures eliminados, ocho servicios saludables y solo gateway:8080 publicado. Revisión enfocada de permisos y secretos aprobada. Evidencias y límites operativos en validation.md, production.md y ADR-010. Sin pendientes, commit ni push.

## Entrega de Fase 5 — Rendimiento y Envasado (2026-09-25)

Fase 5 queda completada bajo el modelo confirmado: cada presentación vendible es un artículo `FINISHED_PRODUCT` independiente, controlado en `UNIT` y con capacidad nominal explícita. No se creó `Presentation`. El contenido neto por unidad se registra en cada envasado y se valida por dimensión; no se deduce de la capacidad del recipiente.

Production registra rendimiento y coordina operaciones persistentes `YIELD` y `PACKAGE`. Inventory las aplica en transacciones locales serializables e idempotentes: `PRODUCTION_IN` para granel; `PRODUCTION_OUT` y `PACKAGING_OUT` para consumos; `PACKAGED_PRODUCT_IN` para unidades vendibles. El UUID enlaza lote, rendimiento, envasado, materiales y movimientos sin compartir Prisma ni consultar bases ajenas. ADR-011 documenta esta extensión de ADR-002 y ADR-010.

`/produccion/lotes` incorpora formularios de rendimiento y envasado, selectores de catálogo, confirmación previa de efectos de inventario, métricas, mermas, materiales y movimientos. Conserva permisos, estados de carga/error/vacío y conflictos sin pérdida de entradas. Playwright comprobó 1440, 768 y 375 píxeles; el visor local falló por el sandbox, por lo que no se atribuye inspección visual manual.

La preparación de migración aprobó respaldos legibles, clean/upgrade, deploy repetido y conservación. Inventory añade `202609250001_yield_packaging_movements`; Production añade `202609250001_yield_packaging` y `202609250002_packaging_integer_constraint`. Los respaldos Fase 5 tienen SHA-256 `e2bfe4548eddb32e6940325d8e1e79f5d344728d77eceeb1b78e997a643cf3bf` (Inventory) y `ff5ca0a1373b598896bf13fdd3e7ae421c3cb06f9092ac0d3ce654094e0d6a20` (Production).

Cierre: lint, typecheck, 433 pruebas —las 428 heredadas y cinco nuevas—, build y formato aprobados. `test:stack` aprobó 18 grupos y Producción ocho grupos reales con concurrencia, rollback, recuperación, rendimiento, envasado, permisos, persistencia y ledger. El estado de 2026-09-25T18:21:57.217Z confirma datos preservados, fixtures limpios, ledger reconciliado, ocho contenedores saludables y solo gateway:8080 publicado. Sin commit, push ni despliegue externo.

...
try {
  ...
  inventory(
    `if(!/^catalog_probe_[a-f0-9]{16}$/.test(input.schema))throw Error();const rows=await db.$queryRawUnsafe('SELECT count(*)::int AS count FROM "'+input.schema+'"."_prisma_migrations" WHERE finished_at IS NOT NULL');if(rows[0].count!==1)throw Error();`,
    { schema },
  );
